...
ble_gattc_proc_free(struct ble_gattc_proc *proc)
{
    ...
    if (proc != NULL) {
        ...
        if (ble_hs_cfg.eatt && proc->cid != BLE_L2CAP_CID_ATT) {
            ble_eatt_release_chan(proc->conn_handle, proc->cid);
        }
#endif
...
ble_gattc_proc_insert(struct ble_gattc_proc *proc, bool insert_head)
{
    ...
    {
        struct ble_gattc_proc *cur;
        /* Avoid double insertion if proc is already on the active list. */
        STAILQ_FOREACH(cur, &ble_gattc_procs, next) {
            if (cur == proc) {
                ble_hs_unlock();
                return;
...
#if MYNEWT_VAL(BLE_GATTC_PROC_PREEMPTION_PROTECT)
        /* Remove from temp_proc_list only if it is actually present. */
        STAILQ_FOREACH(cur, &temp_proc_list, next) {
            if (cur == proc) {
...
#if MYNEWT_VAL(BLE_GATTC_PROC_PREEMPTION_PROTECT)
static bool
ble_gattc_proc_remove_preempted(struct ble_gattc_proc *proc)
{
    struct ble_gattc_proc *cur;
    bool removed = false;
    ...
    if (!removed) {
        STAILQ_FOREACH(cur, &ble_gattc_procs, next) {
            if (cur == proc) {
                STAILQ_REMOVE(&ble_gattc_procs, proc, ble_gattc_proc, next);
                removed = true;
                break;
            }
        }
...
#if MYNEWT_VAL(BLE_GATTC_PROC_PREEMPTION_PROTECT)
        if (proc != NULL) {
            if (!ble_gattc_proc_remove_preempted(proc)) {
                proc = NULL;
            }
        }
#endif
...
ble_gattc_find_inc_svcs_tx(struct ble_gattc_proc *proc)
{
    ...
    if (proc->find_inc_svcs.cur_start == 0) {
        if (proc->find_inc_svcs.prev_handle >= proc->find_inc_svcs.end_handle ||
            proc->find_inc_svcs.prev_handle == 0xFFFF) {
            /* Range exhausted; nothing more to discover. */
                                    ...
                                    struct os_mbuf **om)
{
    ...
    *om = NULL;
    ...
    /* Check if we've exhausted the handle range before resuming */
    if (proc->find_inc_svcs.prev_handle >= proc->find_inc_svcs.end_handle) {
                                 ...
                                 struct ble_att_read_type_adata *adata)
{
    ...
    if (chr.val_handle <= adata->att_handle ||
        chr.val_handle > proc->disc_all_chrs.end_handle) {
        /* Invalid value handle; terminate procedure. */
        rc = BLE_HS_EBADDATA;
        goto done;
    }

    /* We'll resume after characteristic declaration handle */
    proc->disc_all_chrs.prev_handle = adata->att_handle;

    rc = 0;

done:
    cbrc = ble_gattc_disc_all_chrs_cb(proc, rc, 0, rc == 0 ? &chr : NULL);
    if (rc != 0 || cbrc != 0) {
        return BLE_HS_EDONE;
    } else {
        return 0;
...
ble_gattc_disc_all_chrs_rx_complete(struct ble_gattc_proc *proc, int status)
{
    ...
    /* Per Core Spec, discovery should continue until Attribute Not Found
     * or until the next request would start beyond end_handle. The previous
     * `+ 1 >= end_handle` short-circuit skipped the final spec-mandated
     * READ_BY_TYPE when prev_handle == end_handle - 1.
     */
                                 ...
                                 struct ble_att_read_type_adata *adata)
{
    ...
    if (chr.val_handle <= adata->att_handle ||
        chr.val_handle > proc->disc_chr_uuid.end_handle) {
        /* Invalid value handle; terminate procedure. */
        rc = BLE_HS_EBADDATA;
        goto done;
    }
                                 ...
                                 struct ble_att_find_info_idata *idata)
{
    ...
    if (idata->attr_handle <= proc->disc_all_dscs.prev_handle ||
        idata->attr_handle > proc->disc_all_dscs.end_handle) {

        /* Peer sent descriptors out of order or out of range; terminate. */
        rc = BLE_HS_EBADDATA;
        goto done;
    }
...
{
    if (num == 0) {
        return BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_FOUND);
    } else if (offset >= num) {
        BLE_HS_LOG(ERROR, "%s rc=%d\n", __func__,
                   BLE_HS_ATT_ERR(BLE_ATT_ERR_INVALID_OFFSET));
        return BLE_HS_ATT_ERR(BLE_ATT_ERR_INVALID_OFFSET);
    }
                     ...
                     ble_gattc_db_elem_t *result, uint16_t *count)
{
    ...
    /* Not valid (issue-137): null check here is correct; downstream BLE_ATT_ERR_ATTR_NOT_FOUND conflation is pre-existing, not introduced by this MR */
    if (count == NULL || result == NULL) {
        return BLE_HS_EINVAL;
    }
                             ...
                             struct ble_att_read_type_adata *adata)
{
    ...
    if (attr.om == NULL) {
        rc = BLE_HS_ENOMEM;
        rc = ble_gattc_read_uuid_cb(proc, rc, adata->att_handle, NULL);
    } else {
        rc = 0;
        rc = ble_gattc_read_uuid_cb(proc, rc, adata->att_handle, &attr);
    }
                             ...
                             void *cb_arg)
{
    ...
    if (num_handles < 2 || num_handles > MYNEWT_VAL(BLE_GATT_READ_MAX_ATTRS)) {
        rc = BLE_HS_EINVAL;
        goto done;
    }
    if (handles == NULL) {
                       ...
                       struct os_mbuf *txom)
{
    ...
    if (rc != 0) {
        STATS_INC(ble_gattc_stats, write_no_rsp_fail);
    }
...
ble_gattc_write_reliable_tx(struct ble_gattc_proc *proc)
{
    ...
    mtu = ble_att_mtu_by_cid(proc->conn_handle, proc->cid);
    if (mtu <= BLE_ATT_PREP_WRITE_CMD_BASE_SZ) {
        rc = BLE_HS_ENOTCONN;
        goto done;
    }
    max_sz = mtu - BLE_ATT_PREP_WRITE_CMD_BASE_SZ;
...
ble_gattc_write_reliable_resume(struct ble_gattc_proc *proc)
{
    ...
    rc = ble_gattc_process_resume_status(proc, status);
    if (rc != 0) {
        /* Fire error callback here so ble_gattc_resume_procs (which calls
         * ble_gattc_process_status with call_cb=false) does not silently
         * drop the proc without notifying the application. */
                             ...
                             uint16_t att_handle)
{
    ...
    /* If we have successfully queued any data, and the failure occurred before
     * we could send the execute write command, then erase all queued data
     * BEFORE invoking the application callback to keep peer state consistent.
     * cur_attr is incremented only after an attribute is fully queued; offset
     * is advanced only after a verified Prepare Write Response. BLE_HS_EBADDATA
     * means the peer queued data but local verification failed — cancel anyway.
     */
                                 ...
                                 struct os_mbuf **rxom)
{
    ...
    if (rc != 0) {
        /* resume() already called ble_gattc_write_reliable_err(); skip err:
         * label to avoid firing the application callback a second time. */
        return BLE_HS_EDONE;
    }
...
static int ble_gatts_check_conn_aware(uint16_t conn_handle, bool *aware) {
    ...
    if (!(conn->bhc_gatt_svr.peer_cl_sup_feat[0] & 0x01)) {
        *aware = true;
        return 0;
    }
    ...
    for (i = 0; i < chr_count; i++) {
        if (OS_MBUF_PKTLEN(txom) + OS_MBUF_PKTLEN(tuples[i].value) > mtu && cur_chr_cnt < 2) {
            ...
            if (cur_chr_cnt == 1) {
                 ...
                 * single-value notification: the spec requires at least two
                 * handle-value pairs in ATT_MULTIPLE_HANDLE_VALUE_NTF. */
                os_mbuf_adj(txom, 4);
        ...
        /* Handle */
        rc = os_mbuf_append(txom, &tuples[i].handle, sizeof(uint16_t));
        if (rc != 0) {
            rc = BLE_HS_ENOMEM;
            goto done;
        }
        ...
        /* Length */
        rc = os_mbuf_append(txom, &OS_MBUF_PKTLEN(tuples[i].value),
                            sizeof(uint16_t));
        if (rc != 0) {
            rc = BLE_HS_ENOMEM;
            goto done;
                          ...
                          const uint16_t *chr_val_handles)
{
    ...
    if (num_handles == 0 || chr_val_handles == NULL) {
        return BLE_HS_EINVAL;
    }
    /* Cap VLA size to prevent stack overflow from large num_handles */
    if (num_handles > 32) {
        return BLE_HS_EINVAL;
    }
    ...
    peer_feat = conn->bhc_gatt_svr.peer_cl_sup_feat[0];
    ble_hs_unlock();
...
ble_gattc_rx_err(uint16_t conn_handle, uint16_t cid, uint16_t handle, uint16_t status)
{
    ...
    if (proc != NULL) {
#if MYNEWT_VAL(BLE_GATTC_AUTO_PAIR)
        if (auto_pair_ok && desc.sec_state.encrypted == 0 &&
        ...
        err_cb = ble_gattc_err_dispatch_get(proc->op);
        if (err_cb != NULL) {
            err_cb(proc, BLE_HS_ATT_ERR(status), handle);
        }
        ble_gattc_proc_free(proc);
    }
}
...
ble_gattc_tmo_dispatch_init(void)
{
    ...
    if (ble_gattc_tmo_dispatch != NULL) {
        return 0;
    }
...
ble_gattc_deinit(void)
{
    ...
    {
        struct ble_gattc_proc *proc;
        while ((proc = STAILQ_FIRST(&ble_gattc_procs)) != NULL) {
            STAILQ_REMOVE_HEAD(&ble_gattc_procs, next);
            ble_gattc_proc_free(proc);
        }
#if MYNEWT_VAL(BLE_GATTC_AUTO_PAIR)
        while ((proc = STAILQ_FIRST(&ble_gattc_cached_procs)) != NULL) {
            STAILQ_REMOVE_HEAD(&ble_gattc_cached_procs, next);
            ble_gattc_proc_free(proc);
        }
#endif
